...
- `Agent Runner`：非 Agent，按 `prompt_ref` 选择当前支持的无状态业务处理器；业务提示词只读取 MySQL `prompt_registry`，没有记录时当前步骤失败。当前支持写入 payload 解析和更新/删除候选记录匹配。
- `Validation Node`：非 Agent，负责字段对齐、类型校验、把 search_agent 匹配出的 `record_id` 合并进更新/删除请求、生成确认预览和幂等 key。
