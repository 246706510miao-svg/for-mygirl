...
4. 没有可执行计划时，由 `workflowagent` 生成 `workflow_plan`，再由 `Plan Validator` 校验。
...
- `workflowagent`：Agent，只负责生成 `workflow_plan`，不直接写入飞书。
- `Workflow Executor`：非 Agent，负责解释并执行 `workflow_plan.steps`。
- `Plan Validator`：非 Agent，校验工具名、步骤依赖、写入前置条件和风险等级。
...
- `Agent Runner`：非 Agent，按 `prompt_ref` 加载提示词并调用无状态业务 Agent。
