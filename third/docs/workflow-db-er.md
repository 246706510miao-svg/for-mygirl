...
保存可被 workflowagent 和 Agent Runner 使用的业务 Agent 目录及提示词。`Prompt/runagent/*.yaml` 是维护来源，执行 seed 脚本后写入本表；运行时只读取数据库，不读取文件兜底。`workflowagent` 会读取启用记录中的 `agent_name`、`prompt_key`、`role_name`、`description`、`db_address`、输入输出 schema、`metadata_json`、`version` 和 `enabled` 来选择模板或兼容旧 plan 中的 `kind=agent` 步骤；Agent Runner 再按 `prompt_ref` 读取同一条记录的 `prompt_text`。当前内置 `parse_feishu_record.v1` 用于写入 payload 解析，`parse_feishu_schema_change.v1` 用于字段变更 payload 解析，`search_feishu_record.v1` 用于更新/删除前从候选记录中匹配目标 `record_id`。
...
6. 字段变更 Tool 执行成功后必须立即重新读取字段定义并覆盖缓存，后续记录写入使用刷新后的 schema。
