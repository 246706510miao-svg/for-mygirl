"""tool_DeleteFeishuBitableRecord：删除飞书多维表格记录的工具。"""
...
from .mock_repository import delete_mock_record, read_mock_records
...
# 这个函数是删除记录工具入口，输入和输出都使用 content[0].text。
def run_tool_DeleteFeishuBitableRecord(payload: dict[str, Any]) -> dict[str, list[dict[str, str]]]:
    config = load_config()
    ...
    request = normalize_write_request(OPERATION, explicit_request, original_input, config, table_fields, require_fields=False)
    ...
    try:
        ...
        record, backend = _delete_record(request, config)
...
# 这个函数定位待删除记录；没有 record_id 时用搜索条件定位唯一记录。
def _locate_record_id(request: dict[str, Any], config: Any) -> str:
    ...
    if not lookup_filter.get("conditions"):
        raise FeishuClientError("删除记录前必须提供 record_id，或提供可以唯一定位记录的查询条件。")

    lookup_request = build_lookup_read_request(request)
    records = _search_records_for_delete(lookup_request, config)
    if not records:
        raise FeishuClientError("没有找到可删除的记录，请检查 record_id 或定位条件。")
    if len(records) > 1:
        record_ids = [record.get("record_id") for record in records]
        raise FeishuClientError(f"定位到多条记录，已拒绝删除，请补充更精确条件：{record_ids}")
    return str(records[0]["record_id"])


# 这个函数根据配置选择真实飞书搜索或 mock 搜索来定位删除目标。
def _search_records_for_delete(lookup_request: dict[str, Any], config: Any) -> list[dict[str, Any]]:
...
# 这个函数根据配置选择真实飞书删除或 mock 删除。
def _delete_record(request: dict[str, Any], config: Any) -> tuple[dict[str, Any], str]:
    if config.feishu_use_real or request.get("mock") is False:
        client = FeishuBitableClient(config)
        return client.delete_record(request), "feishu"

    record = delete_mock_record(str(request["record_id"]))
    if not record:
        raise FeishuClientError(f"mock 表中不存在 record_id={request['record_id']} 的记录。")
    return record, "mock"


# 这个函数把删除结果整理成传回 finagent 的 tool_result。
