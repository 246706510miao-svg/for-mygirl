...
def read_mock_records(request: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [_project_fields(record, request["field_names"]) for record in records]
...
def _record_matches(record: dict[str, Any], condition: dict[str, Any]) -> bool:
    ...
    if operator in {"not_empty", "exists"}:
        return bool(actual)
    return True


# 这个函数按 Router 给出的排序规则对 mock 记录排序。
...
# 这个函数按 field_names 投影字段，模拟飞书只返回关心字段的效果。
