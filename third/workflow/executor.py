...
SENSITIVE_KEY_PATTERN = re.compile(r"(secret|token|api[_-]?key|password|authorization|dsn|proxy|base[_-]?url|redis[_-]?url)", re.IGNORECASE)
SENSITIVE_TEXT_PATTERNS = (
    ...
    (re.compile(r"(THIRD_(?:OPENAI|NEWS_FOCUS)_PROXY_URL[\"']?\s*[:=：]\s*[\"']?)([^\s\"',}]+)([\"']?)", re.IGNORECASE), r"\1***\3"),
    (re.compile(r"(THIRD_DEEPSEEK_API_KEY[\"']?\s*[:=：]\s*[\"']?)([^\s\"',}]+)([\"']?)", re.IGNORECASE), r"\1***\3"),
    (re.compile(r"(THIRD_MINIMAX_API_KEY[\"']?\s*[:=：]\s*[\"']?)([^\s\"',}]+)([\"']?)", re.IGNORECASE), r"\1***\3"),
    (re.compile(r"(THIRD_DEEPSEEK_BASE_URL[\"']?\s*[:=：]\s*[\"']?)([^\s\"',}]+)([\"']?)", re.IGNORECASE), r"\1***\3"),
    (re.compile(r"(THIRD_MINIMAX_BASE_URL[\"']?\s*[:=：]\s*[\"']?)([^\s\"',}]+)([\"']?)", re.IGNORECASE), r"\1***\3"),
