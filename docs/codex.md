...
1. 先读 [架构图/01_总架构图.md](架构图/01_总架构图.md)，理解用户端、管理员端、后端、数据库、AI、ASR 和飞书 MCP 的整体关系。
2. 再读 [数据库/codex.md](数据库/codex.md)，理解核心表、状态字段、数据不变量和 API 到表的对应关系。
3. 需要实现前后端联调或 Controller 时，读 [接口文档.md](接口文档.md)。
4. 需要确认一次记录如何从输入走到飞书写入时，读 [活动图/整体新增记录闭环活动图_优化版.md](活动图/整体新增记录闭环活动图_优化版.md)。
5. 需要实现用户新增记录交互时，读 [活动图/用户新增记录活动图_优化版.md](活动图/用户新增记录活动图_优化版.md)。
6. 需要实现确认、生成 payload、校验、写入和失败重试时，读 [活动图/AI理解总结生成MCP格式活动图_优化版.md](活动图/AI理解总结生成MCP格式活动图_优化版.md) 与 [活动图/记录状态流转图_优化版.md](活动图/记录状态流转图_优化版.md)。
7. 需要实现管理员后台、异常处理或每日内容配置时，读 [活动图/管理员相关活动图_优化版.md](活动图/管理员相关活动图_优化版.md)。
8. 需要部署或判断边界时，读 [架构图/02_部署边界图.md](架构图/02_部署边界图.md)。
...
| 文档 | 用途 |
|---|---|
| [接口文档.md](接口文档.md) | 前后端 API 设计，包含统一响应、鉴权、幂等字段、用户端接口、记录会话接口、管理员端接口、接口到表映射和 MVP 优先级。 |
